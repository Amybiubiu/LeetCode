class Solution {
public:
    //分类找递推公式，且用三维数组，第n次掷骰子，出现点数为i，且之前重复出现i的次数为m次的这种状态有多少数量，
    //还真的不太会，感觉这算一个组合问题
    int dieSimulator(int n, vector<int>& rollMax) {
        const int divider = 1e9 + 7;
        int sum = 0;
        vector<vector<vector<int>>> dp(n,vector<vector<int>>(7,vector<int>(16,0)));
        for(int i= 0;i < 7; i++){
            dp[0][i][1] = 1;
        }
        for(int i = 1;i < n ;i++){ //第i+1次投掷骰子
            for(int j = 1;j < 7; j++){ //这次出现点数为j
                for(int k = 1; k < 7; k++){//记录上一次投出的点数为k；
                        if(k != j){
                            for(int t = 1; t <= rollMax[k-1]; t++){ //very vital
                                dp[i][j][1] += dp[i-1][k][t];
                                dp[i][j][1] %= divider;
                            }
                        }else{
                            for(int t = 1; t < rollMax[k-1]; t++){
                                dp[i][j][t+1] += dp[i-1][k][t];//
                                dp[i][j][t+1] %= divider;
                            }
                        }
                }
            }
        }
        
        for(int i = 1; i < 7; i++){
            for(int j = 1; j <= rollMax[i-1]; j++){
                 sum = (sum + dp[n-1][i][j])%divider;
            }
        }
        return sum;
    }
    
};
